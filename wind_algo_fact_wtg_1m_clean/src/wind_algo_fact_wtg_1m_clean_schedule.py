...
IN_TABLE_NAME = "wind_dw_fact_wtg_1m"
OUT_TABLE_NAME = "wind_algo_fact_wtg_1m_clean"
...
def run_stream_task(hadoop_home, hdfs_root, day, program_base):
    stream_cmd = "hadoop jar %s/share/hadoop/tools/lib/hadoop-streaming-2.6.0.jar -D mapreduce.job.reduces=0 " \
                 ...
                 "-files %s/%s.py," \
                 "hdfs://titan/%s/bi/logs/conf/wind/%s/wind_algo_fact_wtg_1m_clean.conf," \
                 "hdfs://titan/%s/bi/logs/conf/wind/%s/wind_algo_fact_wtg_1m_clean_input_schema.conf " \
