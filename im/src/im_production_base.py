...
def round_production(df):
    time_condition = df[STG_FACT_WTG_10M_DATATIME].apply(
        lambda x: x.minute % 10 == 0 and x.second == 0)
...
def fill_in_data(turbine_real_all):
    columns = turbine_real_all.columns
    start = pd.DataFrame(None, index=pd.DatetimeIndex([execute_day]))
    turbine_real_all = pd.concat([start, turbine_real_all])
    ...
    turbine_real_all[ACTUAL_PRODUCTION_PER_SECOND] = np.round(turbine_real_all[CAL_ACTIVE_APPRODUCTION_10M] / (10 * 60),
                                                              decimals=5)
    turbine_real_all = turbine_real_all.asfreq('1s', method='bfill')
    turbine_real_all[STG_FACT_WTG_10M_DATATIME] = turbine_real_all.index
    round_production(turbine_real_all)
    return turbine_real_all[turbine_real_all.index > execute_day][columns]
...
def write_data(turbine, turbine_base):
    ...
    make_dirs(out_dir)
    if debug:
        turbine_base.to_csv(os.sep.join([out_dir, str(turbine)]), index=False)
    else:
