...
def init(argv):
    ...
    hive_db = argv[3]
    ocean_home = argv[4]
    return hadoop_home, hdfs_root, day, hive_db, ocean_home


def rm_dest_hdfs_dir(hdfs_root, day):
    rm_hdfs_dir_cmd = "hadoop fs -rmr /%s/bi/logs/wind/wtg_minute_daily/hp_date=%s" % (hdfs_root, day)
...
def run_stream_task(hadoop_home, hdfs_root, day, ocean_home):
    stream_cmd = "hadoop jar %s/share/hadoop/tools/lib/hadoop-streaming-2.6.0.jar " \
                 "-D mapreduce.job.reduces=0 " \
                 "-D mapreduce.job.name=\"wtg_minute_daily_%s\" " \
                 "-files %s/logs/wind/wtg_minute_daily/wtg_minute_daily.py," \
                 "hdfs://titan/%s/bi/logs/conf/wind/wtg_minute_daily/schema.conf," \
                 "hdfs://titan/%s/bi/logs/conf/wind/wtg_minute_daily/clean.conf " \
                 "-input /%s/bi/logs/wind/wtg_minute_raw/hp_date=%s " \
                 "-output /%s/bi/logs/wind/wtg_minute_daily/hp_date=%s " \
                 "-mapper 'python wtg_minute_daily.py'" \
                 % (hadoop_home, day, ocean_home, hdfs_root, hdfs_root, hdfs_root, day, hdfs_root, day)
    print stream_cmd
...
def add_partition(day, hive_db, hdfs_root):
    ...
    add_partition_cmd = 'hive -e "use %s; ' \
                        ...
                        'location \'/%s/bi/logs/wind/wtg_minute_daily/hp_date=%s\'"' % (
                            hive_db, table_name, day, hdfs_root, day)
...
if __name__ == "__main__":
    hadoop_home, hdfs_root, day, hive_db, ocean_home = init(sys.argv[1:])
    rm_dest_hdfs_dir(hdfs_root, day)
    res = run_stream_task(hadoop_home, hdfs_root, day, ocean_home)
    if res != SUCCESS:
        exit(res)
